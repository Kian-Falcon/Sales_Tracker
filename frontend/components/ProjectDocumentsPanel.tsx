...
import type { Department, ProjectDocument, ProjectDocumentType } from "@/lib/types";
...
export function ProjectDocumentsPanel({
  ...
  const uploadType: ProjectDocumentType = viewerDepartment === "R&D" ? "costing_boq" : "boq";
  ...
  const handleUpload = () => {
    ...
    startTransition(() => {
      void (async () => {
        try {
          const uploadedDocument = await uploadProjectDocument(projectId, selectedFile, uploadType);
          setSelectedFile(null);
          onDocumentUpload?.(uploadedDocument);
          pushToast({
            tone: "success",
            title: isCostingBoqMode ? "Costing BOQ uploaded" : "BOQ uploaded",
            description: isCostingBoqMode
      ...
      <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
        <div className="space-y-2">
          ...
          <h2 className="text-2xl font-semibold text-ink">
            {isCostingBoqMode ? "Costing BOQ handoff" : "BOQ and project files"}
          </h2>
          <p className="max-w-2xl text-sm text-ink/60">
            {isCostingBoqMode
              ? "Upload the finalized costing BOQ here. R&D costing stages stay locked until this handoff file is attached."
        ...
        {canUpload ? (
          <div className="w-full rounded-[24px] border border-dashed border-ink/15 bg-surface-muted/35 p-4 lg:max-w-md">
            ...
            <div className="mt-3 flex flex-wrap items-center gap-3">
              ...
              >
                {pending ? "Uploading..." : isCostingBoqMode ? "Upload costing BOQ" : "Upload BOQ"}
              </button>
...
function formatDocumentTypeLabel(documentType: ProjectDocumentType) {
  if (documentType === "costing_boq") {
    return "Costing BOQ";
  }
